...
func NewService(repo *Repository) *Service { return &Service{Floor: 0.168, Repo: repo} }
...
func (s *Service) Run(c Command) result.Result[Entity] {
	...
	e := Entity{ID: uuid.NewString(), TenantID: c.TenantID, Domain: "audit1517", Score: score, Tags: c.Tags, At: time.Now().UTC()}
	s.Repo.Save(e)
	return result.Ok(e)
}
